...
def MSTD(X , m , M , step , n_runs , whiten = True , max_iter = 2000 , n_jobs = -1):
    ...
    for i in range(m , M+step , step): #uncomment if you don't want to use tqdm (and comment the line above !) #JM on 12152020: swith because error "'FloatProgress' object has no attribute 'style'" when calling MSTD
