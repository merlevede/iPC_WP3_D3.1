...
The goal is to identify robust mechanims active in the tumor of interest in iPC, which are Ewing sarcoma, Medulloblastoma, Neuroblastoma, Hepatoblastoma and leukemias. We provide here the data needed to perform the analyses on Ewing Sarcoma tumor type. Data for the other tumor types can be accessed through the following link (available as soon as they will be ready to use).

We suggest the readers to start by reading the methods, discovered the example datasets and read the recommendations for usage.
...
Step 1 and 2 were performed using an implementation in python of Stabilized ICA developped by Nicolas Captier, a team member of Andrei Zinovyev team: https://github.com/ncaptier/Stabilized_ICA. We adapted this notebook to the needs of iPC project. 
...
Step 1 was performed using python jupyter notebook and can be found here. Inputs are normalized log data.

## Step 2: Reciprocal best hit analysis
Using different datasets of a same tumor type should allow to identify a same signal or pathway several times. Moreover, a process can be shared across different technologies or level of sequencing (e.g. bulk, single cell), or even between different data types (patient, cell lines, PDX or normal samples). Thus, an important and reliable mechanism should be identified, maybe not in every, but in most of or several of the datasets. Less "contributive" processes are identified in less datasets.

To achieve this goal and identify robust communities, we took advantage of Reciprocal Best Hit (RBH) analysis. RBH was introduced by Cantini et al. 2019 in the context of omics data analysis. The idea is that there is at most one best match between any components of 2 datasets. Correlation is used between 2 identified metagenes as being a best match to give a weight to this relationship. 

Step 2 was performed using python jupyter notebook and can be found here. Inputs are metagenes, generated at step 1.


## Step 3: Community detection and weighted metagenes
This step is divided into 2 actions. First, the detection of communities.

Starting from RBH analysis results, it is now possible to identify groups reflecting the same processes across various datasets and collapse them into a community. A community is thus simply an ensemble of components sharing biological properties. 
...
The second action is the construction of the weighted metagenes, which are representatives of the communities. Knowing which metagenes are composing a community, we averaged their weights after making sure that the orientation of each metagene is correct. Indeed, in ICA, weights can be positive and negative. The weighted metagenes can now be furthered analyzed.

Step 3 was performed using R and the code in Rmd is provided here. Inputs are the results of reciprocal best hits, obtained at step 2. The json file should be converted to csv file, using cytoscape (https://cytoscape.org/) for example.
...
1. Summarize the information at the level of a community: how many components does it contain? how many single cell / bulk datasets? how many patients/cell lines/PDX/controls datasets.
2. Identify the driver genes and perform over representation analyses on them
...
Step 4 was also performed using R and the code in Rmd is provided here. Inputs are:
...
These datasets log normalized are provided in folder data and zipped subfolder ES.
...
## Step 2: RBH
Once you have run the matrix factorizations on the datasets you wanted, you can run the RBH analysis (step 2) which is available in the last part of the python notebook.
...
You simply can run mcl_and_weighted_metagenes. The inputs needed are:

* rbh csv file
* metagenes files

Output is the weigthed metagenes
...
You simply can run Reports_communites_ES.Rmd which uses template_pathway_analysis.Rmd. 
The outputs are an html report describing the communities.
